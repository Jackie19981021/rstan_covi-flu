...
parameters {
  ...
  real<lower=0> rho12;       // COVID-19患者对流感的暂时免疫率
  real<lower=0> rho21;       // 流感患者对COVID-19的暂时免疫率
...
model {
  ...
  rho12 ~ normal(0.001, 0.05);
  rho21 ~ normal(0.01, 0.05);
  //I2 ~ normal(5, 5);
  // 似然函数
  for (t in 1:T) {
...
generated quantities {
    ...
    for (t in 1:T) {
        S_covid[t] = S[t];  // Assuming S[t] already calculated for COVID-19
        ...
        S_flu[t] = S[t];  // Assuming S[t] also used for Influenza, adjust if separate S needed
